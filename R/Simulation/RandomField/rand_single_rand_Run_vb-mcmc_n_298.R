...
if(MCMC){
        ...
        sim.para <- list(
          ...
          Nt     = 5,
          px     = length(X_vars),
          alpha  = c(5, rep(-1, length(X_vars) - 1)),
          nugget = 1e-2, # SD
  ...
  load(paste0("./result/single_random/sim_random_MCMC_TRUE_298.RData"))
  Train.village.ID <- unique(sim_Data$Village_ID[sim_Data$Simu == "Train"])
}
...
for(r in 1:length(region_flags)) {
  ...
  Beta.list[[r]] <- c(as.vector(coef(lm(var.log.log.Prevalence ~
                                          exp(log.log.mean.Intensity) +
                                          exp(log.log.var.Intensity), data = Var.variables)))[1], 0, 0)
...
Para.List <- lapply(seq_len(Py), function(py){
  ini.sigma.sq <- 1e0
  list(
    beta  = list(mu.beta = c(6, rep(0, length(X_vars) - 1))),
    obs.sigma.sq = list(mu.sigma.sq = ini.sigma.sq, a = 2, b = 1),
    st.sRF = list(
      Initial = list(
        rho.v = list(mu.rho.v = rep(1e-1, res.num),
                     var.rho.v = rep(1e-2, res.num)),
...
Tab <- paste0("./result/single_random")
if (!dir.exists(Tab)) {
  dir.create(Tab, recursive = TRUE)
}
save(CV_Ranalysis, run.time, sim_Data, file = paste0(Tab, "/sim_random_MCMC_", MCMC,"_298.RData"))
