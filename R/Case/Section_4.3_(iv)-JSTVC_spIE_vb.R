...
#-----------------------------------------
#1. Setting of JSTVC, accounting for spatial IEs
#-----------------------------------------
# range: spatial IEs (range = 10km/30km/50km)

for(range in c(10, 30, 50)){
X_vars <- c("Intercept",
            paste0("CWT_", 1:4), paste0("SBT_", 1:4),
            "IEt.CWT", "IEt.SBT",
            paste0("IEs.CWT.sp.Neigh.500.", range),
            paste0("IEs.SBT.sp.Neigh.500.", range)
            )
Tab.Name <- paste0(Tab, "/JSTVC_Xi_IEt_IEs_VB_pow_decay_loglog_", length(X_vars), "_", range, ".RData")
#-----------------------------------------
#-----------------------------------------
set.seed(1234)
#-----------------------------------------
