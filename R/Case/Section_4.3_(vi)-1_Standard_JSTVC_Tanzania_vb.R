...
Tab.Name <- paste0(Tab, "/Tanzania_JSTVC_Xi_IEt_VB_pow_decay_loglog_", length(X_vars), ".RData")
#-----------------------------------------
#-----------------------------------------
set.seed(1234)
#-----------------------------------------
...
#-----------------------------------------
region_flags <- c( "Western", "Eastern")
Tan_indices  <- lapply(region_flags[1:2], function(f) which(Site$flag == f))
...
# setDT(Site)
#
# Site$Flag <- ifelse(Site$Village_ID %in% Test, "test", "train")
#
#
# Score_Data$Flag <- ifelse((Score_Data$Village_ID %in% c(Test)), "test", "train")
...
for(r in 1:length(region_flags)) {
  Temp <- Site[Tan_indices[[r]], c("Village_ID","LAT","LON")] %>%
  ...
  G.basic.data[[r]] <- regCreateGridm(
    sample.coords   = Site[Tan_indices[[r]], ],
    adjacent.matrix = G.mat,
    loc             = c("LAT","LON"),
    pred.coords     = Site[Tan_indices[[r]], ],
    BAUs.Dist       = Tanzania.Dist.c[Tan_indices[[r]], Tan_indices[[r]]],
    R.sqrt          = 0,
    site.id         = "Village_ID",
    ch              = 50,
    method          = "Wenland",
    H.Grid_dist     = Tanzania.Dist.c[Tan_indices[[r]], Tan_indices[[r]]],
...
Para.List <- lapply(seq_len(Py), function(py){
  list(
    obs.sigma.sq = list(mu.sigma.sq = 1e2, a = 2, b = 1),
    st.sRF = list(
      Initial = list(
