...
Tab.Name   <- paste0(Tab, "/Kenya_JSTVC_Xi_IEt_VB_pow_decay_loglog_", length(X_vars), ".RData")
#-----------------------------------------
...
#-----------------------------------------
region_flags <- c("Northwestern", "Northeastern", "Southern")
Ken_indices  <- lapply(region_flags[1:3], function(f) which(Site$flag == f))
#-----------------------------------------
Y_vars    <- c("Prevalence")
...
for(r in 1:length(region_flags)) {
  Temp <- Site[Ken_indices[[r]], c("Village_ID","LAT","LON")] %>%
  ...
  G.basic.data[[r]] <- regCreateGridm(
    sample.coords   = Site[Ken_indices[[r]], ],
    adjacent.matrix = G.mat,
    loc             = c("LAT","LON"),
    pred.coords     = Site[Ken_indices[[r]], ] ,
    BAUs.Dist       = Kenya.Dist.c[Ken_indices[[r]], Ken_indices[[r]]],
    R.sqrt          = 0,
    site.id         = "Village_ID",
    ch              = 50,
    method          = "Wenland",
