...
required_ate_objects <- c("JSTVC_non_decay.ATE")
...
Title.1 <- TeX(" With time-varying coefficients for IEs")
ATE.1 <- colSums(JSTVC_non_decay.ATE)
...
pdf(
  file = file.path(figure_dir, "FigS21_Ranks_non_decay.pdf"),
  width = 8.5,
  height = 5
)
...
layout(
  ...
  widths = c(panel.width.1, panel.width.2)
)
...
mtext("Ranking of arms: ", side = 1, line = 1.1, adj = 0, cex = 1.5)
mtext(
  paste0(
...
for (i in seq_along(group_names)) {
  ...
  text(
    x = causal_effect.1[i] + ifelse(causal_effect.1[i] < 0,
                                    0.02,
                                    -0.01),
    y = y_center,
    labels = sprintf("%.3f", causal_effect.1[i]),
    col = "black",
    adj = 0.4
  )
}
