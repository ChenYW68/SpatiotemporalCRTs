...
Title.1 <- TeX("(A) Standard JSTVC")
ATE.1 <- colSums(JSTVC.ATE)

Title.2 <- TeX("(B) JSTVC$_{-\\xi}$")
ATE.2 <- colSums(JSTVC_xi.ATE)

Title.3 <- TeX("(C) JSTVC$_{mixed}$")
ATE.3 <- colSums(JSTVC_mixed.ATE)

ATE.4 <- True.[5, ] - True.[1, ]
Title.4 <- TeX("(D) Reduction-based")
...
panel.width <- c(0.55, 0.45, 0.35, 0.3, 0.5)

pdf(file = file.path(figure_dir, "Fig5_Ranks.pdf"), width = 15, height = 5)
...
mtext("Ranking of arms: ", side = 1, line = 1.1, adj = 0.06, cex = 1.5)
mtext(
  paste0(
...
axis(1, labels = FALSE, tck = 0)
mtext(Title.2, side = 3, line = 2.5, adj = 0.1, cex = 1.3)
mtext(expression(ATE == DE + IE), side = 3, line = 0.2, adj = 0.95, cex = 1)
mtext(
  paste0(
...
mtext(expression(ATE), side = 3, line = 0.2, adj = 0.95, cex = 1)
mtext(
  paste0(
...
mtext(expression(ATE == Y[5] - Y[1]), side = 3, line = 0.2, adj = 0.95, cex = 1)
mtext(
  paste0(
...
usr <- par("usr")
label_x <- usr[1] + 0.1
y.x <- c(-0.22, 0.45, 1.12, 1.8, 2.47, 3.15)
