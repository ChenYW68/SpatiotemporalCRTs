...
Title.1 <- TeX(" (A) Using Tanzania's data only")
ATE.1 <- colSums(JSTVC_Tanzania.ATE)

ATE.2 <- True.Tanzania[5, ] - True.Tanzania[1, ]
Title.2 <- " (B) Reduction-based"
...
panel.width <- c(0.75, 0.6, 0.6)

pdf(file = file.path(figure_dir, "FigS18_Ranks_Tanzania.pdf"), width = 10, height = 5)

layout(matrix(c(1, 2, 3), nrow = 1, ncol = 3), widths = panel.width)
...
mtext("Ranking of arms: ", side = 1, line = 1.1, adj = 0.08, cex = 1.5)
mtext(
  paste0(
...
axis(1, labels = FALSE, tck = 0)
mtext(Title.2, side = 3, line = 2.8, adj = 0.1, cex = 1.3)
