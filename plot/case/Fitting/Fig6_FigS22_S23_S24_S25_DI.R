...
if(case %in% c(1, 2, 3)) {
  ...
  DI <- summarise_pairwise_ate_differences(JSTVC.ATE)
  ...
  DI.mcmc <- summarise_pairwise_ate_differences(JSTVC.ATE.mcmc)
} else if(case == 4) {
  JSTVC.ATE <- compute_spatiotemporal_ate_draws(
    score_data = Score_Data,
    fit_object = vb_fit,
  ...
  for(i in 1:5)
  {
    for(j in (i + 1):6){
      DI.mcmc <- rbind(DI.mcmc, data.frame(k1 = i, k2 = j,
                                           mu = mean(ATE.mcmc[i,] - ATE.mcmc[j,]),
                                           sigma.sq = var(ATE.mcmc[i,] - ATE.mcmc[j,])))
    }
  }
}
...
if(case == 1){
  ...
  for (i in 1:nrow(df)) {
    ...
    plot_two_normal_distributions(mean1 = df.mcmc$mu[i],
