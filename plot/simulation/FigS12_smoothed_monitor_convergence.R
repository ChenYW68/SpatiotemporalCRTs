...
pdf(file  = "./figure/FigS12_smoothed_monitor_convergence.pdf",
    width = 18,
    height = 7)
...
plot(CV_Ranalysis[[1]]$detailed.Para.List$ELBO/1e5,
     ...
     xlim = c(1, 230),
     ylim = c(-1, max(CV_Ranalysis[[1]]$detailed.Para.List$ELBO/1e5)),
     axes = FALSE,
     frame.plot = FALSE)
...
plot(CV_Ranalysis$detailed.Para.List$ELBO/1e5,
     ...
     ylab = expression(Evidence~Lower~Bound~"(×"~10^{5}*")"),
     xlim = c(1, 230),
     ylim = c(-1, max(CV_Ranalysis$detailed.Para.List$ELBO/1e5)),
     axes = FALSE,
     frame.plot = FALSE)
